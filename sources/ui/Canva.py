...
class Canva(pygame.sprite.Sprite):
            ...
            if pygame.mouse.get_pressed(3)[0] and self.rect.collidepoint(pygame.mouse.get_pos()):  
                match self.selectedTool:
                    ...
                    case "colorpicker":
                        ...
                        except Exception as Error: # In the case where the mouse is not on the Canva
                            raiseAnError(Error)
    ...
    def holyBucket(self, x, y, color, toile):
        ...
        if baseColor == color:
            return
        PAS = set() # Equivalent to the list but faster (optimization)
        pixels = deque() # Same thing, but it's a line
        pixels.append((x, y))
